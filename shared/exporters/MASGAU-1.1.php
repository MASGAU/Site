...
class Exporter extends MASGAUBase {
    ...
    public function exportGameVersion($game, $version) {
        ...
        if ($version->region != null) {
            $new_game->appendChild($this->xml->createAttribute("country"))->
        ...
        if ($version->title != null) {
            $new_game->appendChild($this->createElement("title", self::cleanUp($version->title)));
        } else {
            $new_game->appendChild($this->createElement("title", self::cleanUp($game->title)));
        }
        ...
        foreach ($version->locations as $location) {
            ...
            if ($location->deprecated) {
                $tag->appendChild($this->xml->createAttribute("read_only"))->
                        appendChild($this->createTextNode("true"));
            }
        }
        ...
        if ($version->override_virtualstore) {
            ...
            $tag->appendChild($this->xml->createAttribute("override"))->
                    appendChild($this->createTextNode("yes"));
        }
        ...
        foreach ($version->contributors as $contributor) {
            $new_game->appendChild($this->createElement("contributer", $contributor));
        }
        return $new_game;
    }
